...
import { FiTrash2, FiMinus, FiPlus, FiShoppingCart, FiArrowRight, FiTag, FiPackage, FiTruck } from 'react-icons/fi';

export default function CartPage() {
  const router = useRouter();
  const { items, updateQuantity, removeItem, clearCart, getSubtotal, getDiscount, getTotal, discountCode, applyDiscountCode, removeDiscountCode } = useCartStore();
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-primary/10 to-background">
        <div className="container-custom py-16">
          <div className="text-center max-w-lg mx-auto">
            ...
            <h1 className="text-4xl font-bold mb-4 bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">Your cart is empty</h1>
            <p className="text-text-secondary mb-8 text-lg">Discover amazing products and start shopping today!</p>
            <Link href="/shop" className="inline-flex items-center gap-2 bg-gradient-to-r from-accent to-accent-light text-white px-8 py-4 rounded-xl font-medium hover:scale-105 transition-transform shadow-lg shadow-accent/20">
        ...
        <div className="mb-12">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-text-secondary">{items.length} {items.length === 1 ? 'item' : 'items'} in your cart</p>
            </div>
          </div>
        </div>
              ...
              >
                <div className="flex gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-white font-semibold text-lg mb-2 group-hover:text-accent transition-colors truncate">{item.name}</h3>
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        {item.color && (
                          <span className="inline-flex items-center gap-1 bg-background/50 text-text-secondary text-xs px-3 py-1 rounded-full border border-gray-700/50">
                            <div className="w-3 h-3 rounded-full border border-gray-600" style={{ backgroundColor: item.color.toLowerCase() }}></div>
                            {item.color}
                          </span>
                        )}
                        {item.size && (
                    ...
                    <div className="flex items-baseline gap-2">
                      <p className="text-accent font-bold text-2xl">${(item.price * item.quantity).toFixed(2)}</p>
                      <span className="text-text-secondary text-sm">${item.price.toFixed(2)} each</span>
                    </div>
                  </div>
            ...
            <div className="bg-gradient-to-br from-primary/80 to-primary backdrop-blur-xl border border-gray-800/50 rounded-2xl p-6 sticky top-8 shadow-2xl">
              ...
              <div className="space-y-3 mb-8">
                <div className="flex justify-between items-center py-2">
                  <span className="text-text-secondary text-sm">Subtotal ({items.length} items)</span>
                  <span className="text-white font-semibold text-lg">${getSubtotal().toFixed(2)}</span>
                </div>
                ...
                <div className="flex justify-between items-center py-2">
                  ...
                  <span className="font-semibold text-green-400 text-sm">FREE</span>
                </div>

                <div className="border-t border-gray-700/50 pt-4 mt-4 flex justify-between items-center">
                  <span className="text-white text-lg font-bold">Total</span>
                  <div className="text-right">
                    <div className="text-accent text-3xl font-bold">${getTotal().toFixed(2)}</div>
                    {discountCode && (
                      <div className="text-text-secondary text-xs line-through mt-1">${getSubtotal().toFixed(2)}</div>
                    )}
                  </div>
                </div>
              </div>
                  ...
                  <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/50 rounded-xl p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-green-400 font-bold text-lg">{discountCode.code}</div>
                          <div className="text-green-400/70 text-sm">-{discountCode.percentage}% discount applied</div>
                        </div>
                      </div>
                      <button 
